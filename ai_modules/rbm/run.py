...
data = {'x_train': np.array(raw[:split]), 'x_test': np.array(raw[split:])}
...
if __name__ == '__main__':
    ...
    plot_image_grid(np.array(samples), image_shape=(4, 4), n_pictures=16, save=False)
    ...
    plot_input_sample(test_input, reconstruction[0], image_shape=(4, 4), save=False)

    # 4 — reconstruction cross entropy with inline plot
    rbm.reconstruction_cross_entropy(data['x_test'][:5], plot=True)
