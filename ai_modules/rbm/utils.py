...
def plot_image_grid(
    ...
    save=True,
    row_titles=None,
    name='grid' + datetime.datetime.now().strftime('%Y%m%d-%H%M%S') + '.pdf',
):
    """
    ...
    for idx, ax in enumerate(axes.flat):
        ...
        sns.heatmap(np.reshape(array[idx, :], image_shape), ax=ax, vmin=vmin, cmap=cmap, cbar=False, vmax=vmax, square=True)

    plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0.03, hspace=0.03)
    fig.colorbar(im2, ax=axes.ravel().tolist(), shrink=0.95)
...
def plot_single_image(image_array, image_shape, save=True, name='image' + datetime.datetime.now().strftime('%Y%m%d-%H%M%S') + '.pdf'):
...
def plot_input_sample(
    input_array, sample_array, image_shape, save=False, name='input_sample' + datetime.datetime.now().strftime('%Y%m%d-%H%M%S') + '.pdf'
):
